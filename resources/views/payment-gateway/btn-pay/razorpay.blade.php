...
<head>
    ...
    <title>Pay with {{ $output['gateway_currency']->name }}</title>
</head>
<body> 
    ...
    <script>
        var options = {
            ...
            "amount": "{{ floatval($output['amount']->total_payable_amount) }}", // Amount is in currency subunits. Default currency is INR. Hence, 50000 refers to 50000 paise
            "currency": "{{ $output['gateway_currency']->currency_code }}",
            "name": "{{ $basic_settings->site_name }}", //your business name
            "description": "Payment With " + "{{ $output['gateway_currency']->name }}",
